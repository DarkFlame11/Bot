...
async def run_search(query, limit=10):
    async with get_db() as db:
        ...
        try:
            ...
            if res: return res
        except Exception as e: logging.error(f"Err1: {e}")
        ...
        try:
            ...
            return comb[:limit]
        except Exception as e: logging.error(f"Err2: {e}")
    return []
...
async def imp_track(m: types.Message):
    ...
    async with get_db() as db:
        ...
        await db.execute("INSERT INTO tracks (title, artist, file_id) VALUES (?,?,?)", (t,a,f))
        await db.commit()
...
async def dpl(c: types.CallbackQuery):
    ...
    async with get_db() as db:
        ...
        await db.execute("DELETE FROM playlist_tracks WHERE playlist_id=?", (pid,))
        await db.commit()
...
async def dt(c: types.CallbackQuery):
    ...
    tid = int(c.data.split("_")[1])
    async with get_db() as db:
        cur = await db.execute("SELECT title, artist FROM tracks WHERE id=?", (tid,))
