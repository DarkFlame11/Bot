...
async def imp_track(m: types.Message):
    ...
    async with pool.acquire() as conn:
        try:
            ...
            await m.answer("❌ Ошибка при сохранении")
...
async def sf(m: types.Message):
    ...
    async with pool.acquire() as conn:
        try:
            ...
            await m.answer("❌ Ошибка при загрузке избранного")
...
async def top(m: types.Message):
    ...
    try:
        ...
        md = ["🥇","🥈","🥉"]
        lines = [
            f"{md[i-1] if i<=3 else f'{i}.'} {html.escape(format_track(t['artist'], t['title']))} — {t['plays']} 🎧" 
            for i, t in enumerate(res, 1)
        ]
        await m.answer("🔥 <b>Топ:</b>\n\n" + "\n".join(lines), parse_mode="HTML")
    except Exception as e:
        logging.error(f"Ошибка в top: {e}")
        await m.answer("❌ Ошибка при загрузке топа")
...
async def spl(m: types.Message, state: FSMContext):
    ...
    try:
        ...
        await m.answer("❌ Ошибка при загрузке плейлистов")
...
async def rpn(m: types.Message, state: FSMContext):
    ...
    try:
        ...
        await m.answer("❌ Ошибка при создании плейлиста")
...
async def opl(c: types.CallbackQuery):
    try:
        ...
        async with pool.acquire() as conn:
            ...
            tr = await conn.fetch(
                "SELECT tracks.id, tracks.file_id FROM tracks JOIN playlist_tracks ON tracks.id=playlist_tracks.track_id WHERE playlist_tracks.playlist_id=$1",
                pid
            )
...
async def mg(m: types.Message):
    ...
    try:
        ...
        kb = InlineKeyboardMarkup(inline_keyboard=[
            [InlineKeyboardButton(text=f"🗑 {html.escape(format_track(t['artist'], t['title']))}", callback_data=f"del_{t['id']}")] 
            for t in res
        ])
        await m.answer(h, reply_markup=kb)
    except Exception as e:
        logging.error(f"Ошибка в mg: {e}")
        await m.answer("❌ Ошибка при загрузке управления")
...
async def sm(m: types.Message, state: FSMContext):
    ...
    if not res:
        ...
        try:
            ...
            await m.answer("❌ Ошибка при поиске")
        return
    
    lines = [f"{i}. {html.escape(format_track(t['artist'], t['title']))}" for i, t in enumerate(res, 1)]
    await m.answer(
        f"🎧 <b>Найдено {len(res)}:</b>\n\n" + "\n".join(lines) + "\n\n👇 Нажми номер:",
        parse_mode="HTML",
        reply_markup=num_buttons([t['id'] for t in res])
    )
